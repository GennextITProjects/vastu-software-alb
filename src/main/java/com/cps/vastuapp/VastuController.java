...
public class VastuController {
    ...
    @FXML
    private ImageView overlayImageView1; // Overlay for 45 Devtas
    @FXML
    private Pane imageContainer;
    @FXML
    private Circle centerMarker; // Blue dot indicating center of the image
    @FXML
    ...
    @FXML
    private ComboBox<String> primaryDropdown;
    @FXML
    private ComboBox<String> secondaryDropdown;
    @FXML
    private ComboBox<String> tertiaryDropdown;
    @FXML
    private ComboBox<String> selectedPointColourDropdown;
    @FXML
    private ComboBox<String> selectedPointBoxColourDropdown;
    // TODO dop box  refrence

    @FXML
    private Slider opacitySlider;
    @FXML
    private Slider opacityCanvasSlider;
    ...
    @FXML
    private GraphicsContext gco;

    private PauseTransition delay;

    private double initialWidth, initialHeight, initialXIV, initialYIV;

    private Rectangle topLeftResizeHandle, topRightResizeHandle, bottomLeftResizeHandle, bottomRightResizeHandle;
    ...
    private void initializeInputs() {
        ...
        angleInput.setOnKeyPressed(event -> {
        ...
        angleInput.setTextFormatter(createAngleTextFormatter()); // TextFormatter added here to restrict input to valid values.
    }
    ...
    private void initializeResizeHandles() {
        ...
        imageContainer.getChildren().addAll(topLeftResizeHandle, topRightResizeHandle, bottomLeftResizeHandle, bottomRightResizeHandle,
                leftResizeHandle, rightResizeHandle, topResizeHandle, bottomResizeHandle);
    ...
    private void updateResizeHandles() {
        if (topLeftResizeHandle != null && topRightResizeHandle != null && bottomLeftResizeHandle != null && bottomRightResizeHandle != null &&
                leftResizeHandle != null && rightResizeHandle != null && topResizeHandle != null && bottomResizeHandle != null) {
...
//                overlayImageView1.setLayoutX(newX); // TODO Needed to cehck with sanket kya kam me aata he
//                overlayImageView1.setLayoutY(newY);
    ...
    private Image renderPdfAsImage(File file) throws IOException {
        try (PDDocument document = Loader.loadPDF(file)) {
            ...
            BufferedImage bufferedImage = renderer.renderImageWithDPI(0, 150); // Render first page at 300 DPI
...
//    }
    ...
    private Image promptCropImage(Image image) {
        ...
        cropDialog.setContentText("Click OK to proceed with cropping, or Cancel to use the original image.");
    ...
    private void handleImport() {
        ...
        fileChooser.getExtensionFilters().addAll(
                new FileChooser.ExtensionFilter("Supported Files", "*.png", "*.jpg", "*.jpeg", "*.pdf", "*.dwg"),
            ...
            Task<Image> loadImageTask = new Task<>() {
                ...
                protected Image call() throws Exception {
                    ...
                    if (fileExtension.equalsIgnoreCase("png") || fileExtension.equalsIgnoreCase("jpg") || fileExtension.equalsIgnoreCase("jpeg")) {
                        //image = new Image(file.toURI().toString(), 2048, 2048, true, true); // Resize if necessary
            ...
            loadImageTask.setOnSucceeded(event -> {
                try {
                    ...
                    if (croppedImportedImage != null) {
                        ...
                        System.out.println("centerXimageCanvas : "+centerXimageCanvas +" centerYimageCanvas: "+centerYimageCanvas);
                    } else {
    ...
    private void handleReset() {
        ...
        Alert alert = new Alert(AlertType.CONFIRMATION, "Are you sure you want to reset?", ButtonType.YES, ButtonType.NO);
    ...
    private void removeResizeHandles() {
        imageContainer.getChildren().removeAll(topLeftResizeHandle, topRightResizeHandle, bottomLeftResizeHandle, bottomRightResizeHandle);
        imageContainer.getChildren().removeAll(leftResizeHandle, rightResizeHandle, topResizeHandle, bottomResizeHandle);
    }
    ...
    private void exportAsImage(Pane imageContainer, File outputFile) {
        ...
        try {
            ...
            showAlert(AlertType.INFORMATION, "Export Successful", "Image exported successfully to: " + outputFile.getAbsolutePath());
    ...
    private void exportAsPDF(Pane imageContainer, File outputFile) {
        ...
        try (PDDocument document = new PDDocument()) {
            ...
            PDImageXObject pdImage = PDImageXObject.createFromFileByContent(convertBufferedImageToFile(bufferedImage), document);
            ...
            showAlert(AlertType.INFORMATION, "Export Successful", "PDF exported successfully to: " + outputFile.getAbsolutePath());
    ...
    private void handleApply() {
        ...
        Task<Image> loadOverlayTask = new Task<>() {
            @Override
            protected Image call() throws Exception {
                ...
                try (InputStream inputStream = getClass().getResourceAsStream(overlayPath)) {
                    ...
                    return new Image(inputStream, 11812 * downscaleFactor, 17970 * downscaleFactor, true, true);
                }
            }
        };

        // Handle success (on the JavaFX application thread)
        loadOverlayTask.setOnSucceeded(event -> {
            Image overlayPathBuilderImage = loadOverlayTask.getValue();
            resetOverlays();

//            if (isLastOption && ! overlayPathBuilder.contains("Circle")  ) {  //TODO Needed to enable after confora
            if (isLastOption  ) {
                ...
                // Center the image
                overlayImageView1.setFitWidth(overlayImageView1.getFitWidth()); // Resize width (optional)
                overlayImageView1.setFitHeight(overlayImageView1.getFitHeight()); // Resize height (optional)
                overlayImageView1.layoutXProperty().bind(imageCanvas.widthProperty().subtract(overlayImageView1.fitWidthProperty()).divide(2));
                overlayImageView1.layoutYProperty().bind(imageCanvas.heightProperty().subtract(overlayImageView1.fitHeightProperty()).divide(2));
...
//                imageCanvas.getId().add(overlayImageView1);
        ...
        loadOverlayTask.setOnFailed(event -> {
    ...
    private void changePointBoxColour() {
        ...
        // Draw the rectangle
        gc.setStroke(colorListForSelectedPointColourDropdown.get(selectedPointBoxColourDropdown.getValue())); // Set color
        ...
        gc.setFill(colorListForSelectedPointColourDropdown.get(selectedPointBoxColourDropdown.getValue())); // Set fill color for the center point
    ...
    private void applyRotation(ImageView imageView, double angle) {
        Platform.runLater(() -> {
            ...
            updateResizeHandles();
            drawCompass(gco, compassCanvas.getWidth(), compassCanvas.getHeight(), angle, compassDirection);
        });
    }
    ...
    public void pointSelection() {
        if (isPointSelectionInProgress) {
            showAlert(Alert.AlertType.INFORMATION, "Point Selection", "Point selection is already in progress. Please complete the current selection first.");
            return;
        }
        ...
        if (overlayImageView1.getImage() != null) {
            ...
            alert.setContentText("An existing selection is detected. Do you want to clear it and start a new selection?");
            AppUtils.setAlertIcon(alert);
        ...
        showAlert(Alert.AlertType.INFORMATION, "Point Selection", "You can start selecting points with a left click. Use the right click to finish or end the selection.");
        isPointSelectionInProgress = true;
        imageCanvas.setCursor(Cursor.HAND);
        imageCanvas.getScene().addEventFilter(KeyEvent.KEY_PRESSED, event -> {
    ...
    private final EventHandler<MouseEvent> pointSelectionHandler = event -> {
        if (event.getButton() == MouseButton.PRIMARY) {
            ...
            showAlert(Alert.AlertType.INFORMATION, "Selection Complete", "You have completed the selection of points.");
        }
    };
    ...
    private void drawLines() {
        if (points.size() > 1) {
            gc.setStroke(colorListForSelectedPointColourDropdown.get(selectedPointColourDropdown.getValue())); // TODO added line point
            gc.setLineWidth(3);
    ...
    private void setupData() {
        ...
        selectedPointBoxColourDropdown.getItems().addAll(colorListForSelectedPointColourDropdown.keySet());
        ...
        squareData.put("DOWN SIDE NORTH", FXCollections.observableArrayList("5_ELEMENTS", "9_ZONES", "DEVTAS", "MARMA_POINTS", "ZONES", "MANDUKA",  "GOOD_ENTRIES_BAD_ZONES"));
        squareData.put("LEFT SIDE NORTH", FXCollections.observableArrayList("5_ELEMENTS", "9_ZONES", "DEVTAS", "MARMA_POINTS", "ZONES", "MANDUKA",  "GOOD_ENTRIES_BAD_ZONES"));
        squareData.put("UP SIDE NORTH", FXCollections.observableArrayList("5_ELEMENTS", "9_ZONES", "DEVTAS", "MARMA_POINTS", "ZONES", "MANDUKA",  "GOOD_ENTRIES_BAD_ZONES"));
        squareData.put("RIGHT SIDE NORTH", FXCollections.observableArrayList("5_ELEMENTS", "9_ZONES", "DEVTAS", "MARMA_POINTS", "ZONES", "MANDUKA" , "GOOD_ENTRIES_BAD_ZONES"));


        Map<String, ObservableList<String>> rectangleData = new HashMap<>();
        rectangleData.put("DOWN SIDE NORTH", FXCollections.observableArrayList("5_ELEMENTS", "9_ZONES", "DEVTAS", "MARMA_POINTS", "ZONES", "MANDUKA",  "GOOD_ENTRIES_BAD_ZONES"));
        rectangleData.put("LEFT SIDE NORTH", FXCollections.observableArrayList("5_ELEMENTS", "9_ZONES", "DEVTAS", "MARMA_POINTS", "ZONES", "MANDUKA",  "GOOD_ENTRIES_BAD_ZONES"));
        rectangleData.put("UP SIDE NORTH", FXCollections.observableArrayList("5_ELEMENTS", "9_ZONES", "DEVTAS", "MARMA_POINTS", "ZONES", "MANDUKA",  "GOOD_ENTRIES_BAD_ZONES"));
        rectangleData.put("RIGHT SIDE NORTH", FXCollections.observableArrayList("5_ELEMENTS", "9_ZONES", "DEVTAS", "MARMA_POINTS", "ZONES", "MANDUKA",  "GOOD_ENTRIES_BAD_ZONES"));
    ...
    private void handlePrimarySelection() {
        ...
        if (selectedCategory != null) {
            ...
            secondaryDropdown.setItems(FXCollections.observableArrayList(shapSelection.get(selectedCategory).keySet()));
    ...
    private void handleTertiarySelection() {
        ...
        if (selectedCategory != null && selectedSubcategory != null && selectedTertiarycategory != null) {
            // Populate tertiary dropdown
            ...
            overlayPathBuilder = "/Directory/" + selectedCategory + "/" + shortNamesForSelection.get(selectedSubcategory) + "/" + selectedTertiarycategory + ".png";
    ...
    private void drawCompass(GraphicsContext gc, double width, double height, double angle, String northDirection) {
        // Lazy initialize off-screen canvas
        if (offScreenCanvas == null || offScreenCanvas.getWidth() != width || offScreenCanvas.getHeight() != height) {
        ...
        for (int degree = 0; degree < 360; degree += 30) {
            ...
            if (degree == 0)
                offGc.fillText("N", x - LABEL_OFFSET_X, y + LABEL_OFFSET_Y);
            else if (degree == 90)
                offGc.fillText("E", x - LABEL_OFFSET_X, y + LABEL_OFFSET_Y);
            else if (degree == 180)
                offGc.fillText("S", x - LABEL_OFFSET_X, y + LABEL_OFFSET_Y);
            else if (degree == 270)
                offGc.fillText("W", x - LABEL_OFFSET_X, y + LABEL_OFFSET_Y);
            else
                offGc.fillText(degree + "°", x - LABEL_OFFSET_X, y + LABEL_OFFSET_Y);
        }
        ...
        offGc.fillOval(centerX - CENTER_CIRCLE_RADIUS, centerY - CENTER_CIRCLE_RADIUS, CENTER_CIRCLE_RADIUS * 2, CENTER_CIRCLE_RADIUS * 2);
    ...
    private void drawNeedle(GraphicsContext gc, double centerX, double centerY, double radius, double angle, String northDirection) {
    ...
    private void showAbout() {
        ...
        ImageView icon = new ImageView((new Image(Objects.requireNonNull(getClass().getResourceAsStream("/icons/app_icon.jpg")))));
        ...
        Task<Image> loadImageTask = new Task<>() {
            ...
            protected Image call() throws Exception {
                ...
                if (fileExtension.equalsIgnoreCase("png") || fileExtension.equalsIgnoreCase("jpg") || fileExtension.equalsIgnoreCase("jpeg")) {
                    ...
                    Platform.runLater(() -> showAlert(Alert.AlertType.INFORMATION, "Information", "DWG files are not currently supported."));
                }
                return image;
            }
        };
    ...
    private void setupDragAndDrop() {
        imageContainer.setOnDragOver(event -> {
            ...
            if (dragboard.hasFiles() && dragboard.getFiles().get(0).getName().matches("(?i).*\\.(png|jpg|jpeg|pdf|dwg)$")) {
            ...
            if (dragboard.hasFiles()) {
                ...
                if (file.getName().matches("(?i).*\\.(png|jpg|jpeg|pdf|dwg)$")) {
                    if (getFileExtension(file).equalsIgnoreCase("dwg")) {
                        showAlert(Alert.AlertType.INFORMATION, "Information", "DWG files are not currently supported.");
                    } else {
    ...
    private void undo() {
        if (isLastOption) {
            showAlert(Alert.AlertType.WARNING, "Undo Disabled", "Undo functionality is disabled after the selection is complete.");
            return;
        }
    ...
    private void redo() {
        if (isLastOption) {
            showAlert(Alert.AlertType.WARNING, "Redo Disabled", "Redo functionality is disabled after the selection is complete.");
            return;
        }
